        ...
        <Row>
            <Card v-show="currentIndex === 1">
                <p slot="title" style="height:32px;">
                    <span style="display:inline-block;float:left;line-height:32px;">基础票</span>
                    <top-btn
                            :btnList="btnList"
            ...
            <Card v-show="currentIndex === 12 || currentIndex === 13">
                <p slot="title" style="height:32px;">
                    <span v-if="currentIndex === 12" style="display:inline-block;float:left;line-height:32px;">新增基础票</span>
                    <span v-if="currentIndex === 13" style="display:inline-block;float:left;line-height:32px;">修改基础票</span>
                </p>
                <detail-form1
                        ref="baseTicket"
                        :isListPage = "false"
            ...
            <Card v-show="currentIndex === 14">
                <p slot="title" style="height:32px;">
                    <span style="display:inline-block;float:left;line-height:32px;">查看基础票</span>
                </p>
                <view-del-detail
                        :isClose = "isClose"
    ...
    export default {
        ...
        data () {
            ...
            const validateDate = (rule, value, callback) => {
                this.formDatas1.note3 = this.$common.getCurrentDate(this.formDatas1.note3);
                this.formDatas1.note4 = this.$common.getCurrentDate(this.formDatas1.note4);
                if (this.formDatas1.int3) {
                    if (!this.formDatas1.note3 && !this.formDatas1.note4) {
                        callback(new Error('分时预约开始和结束日期不能为空'))
                    } else if (this.formDatas1.note3 && !this.formDatas1.note4) {
                        callback(new Error('分时预约结束日期不能为空'))
                    } else if (!this.formDatas1.note3 && this.formDatas1.note4) {
                        callback(new Error('分时预约开始日期不能为空'))
                    } else if (this.formDatas1.note3 && this.formDatas1.note4) {
                        console.log(Date.parse(this.formDatas1.note3));
                        console.log(Date.parse(this.formDatas1.note4));
                        console.log(this.formDatas1.note3);
                        console.log(this.formDatas1.note4);
                        if (Date.parse(this.formDatas1.note3) > Date.parse(this.formDatas1.note4)) {
                            callback(new Error('分时预约开始日期不能大于结束日期'))
                        } else {
                            callback()
                        }
                    }
            ...
            const validateTime = (rule, value, callback) => {
                if (this.formDatas1.int3) {
                    this.formDatas1.timestock.map((item,index) => {
                        let start = this.getSeconds(item.time_start);
                        let end = this.getSeconds(item.time_end);
                        let start1 = '';
                        if (index < this.formDatas1.timestock.length - 1) {
                            start1 = this.getSeconds(this.formDatas1.timestock[index+1].time_start);
                        }
                        if (!item.time_start) {
                            callback(new Error(`第${index+1}项开始时间不能为空`))
                        } else if (!item.time_end) {
                            callback(new Error(`第${index+1}项截止时间不能为空`))
                        } else if (start >= end) {
                            callback(new Error(`第${index+1}项截止时间须大于开始时间`))
                        } else if (!item.stock) {
                            callback(new Error(`第${index+1}项库存不能为空`))
                        } else if (start1 && end > start1) {
                            callback(new Error('分时时段不能重叠'))
                        } else {
                            if (index === this.formDatas1.timestock.length - 1) {
                                callback()
                            }
            ...
            return {
                ...
                btnList: [
                    ...
                    },
                    {
                        ...
                        btnlabel: '价格管理',
                        icon: 'clipboard',
                        routerName: 'baseprice_management',
                ...
                ruleDatas: {
                    ...
                    validityday: [
                        {  required: true, validator: this.$reg.validityday, trigger: 'blur' }
                    ],
                    note3: [
        ...
        methods:{
            ...
            changePage(pageCode) {
                ...
                this.$common.currentPageData(this.baseUrl+'/ticket/ticket/v1/productViewList', this);
            },
            //选择每页条数
            ...
            async listPageSelectedChange (val) {
                ...
                const options = await this.$common.getProductInfoByScenicId(this, val, 1);
            ...
            async getFormDataAdd (obj) {
                ...
                if (aaa) {
                    await this.$common.currentPageData(this.baseUrl+'/ticket/ticket/v1/productViewList', this);
                }
            },
            //增加或减少行
            addOrsubtractRow (num, index) {
                let status = 1;
                if (num === 2) {
                    this.formDatas1.timestock.map(item => {
                        if (index === (this.formDatas1.timestock.length - 1)) {
                            if (!item.time_start || !item.time_end || !item.stock) {
                                this.$common.showMessageModal(this, '请先填写上一时段' , 2);
                                status = 0
                            }
                        }
                    })
                ...
                if (status) {
                    ...
                    if (num === 1) {    //减掉一行时，使减掉行表单验证消失
                        let stock = this.formDatas1.timestock[index-1].stock;
                        this.formDatas1.timestock[index-1].stock = '';
                        setTimeout(() => {   //异步重新赋值，不然表单验证不会消失
                            this.formDatas1.timestock[index-1].stock = stock;
                        },1)
                    }
                }
            },
            ...
            async setAddInitData (type) {
                ...
                this.$common.getParsBeforeSearch(this);
                let index = this.formItemList1.length - 1;
                this.formItemList1[index].colList = [
                    {
                        inputList:[
                            {
                                type: 2,
            ...
            async setEditInitData (type) {
                ...
                await this.getInitSelect();
                let str = `?access_token=${this.Cookies.get('token')}&itickettypeid=${this.currentRowData.id}`;
                this.$http.post(`${this.baseUrl}/ticket/ticket/v1/productEdit${str}`,)
                    .then(res => {
                        if (res.data.code === 200) {
                            ...
                            if (!this.formDatas1.timeList || !this.formDatas1.timeList.length) {
                                ...
                                let index = this.formItemList1.length - 1;
                                this.formItemList1[index].colList = arr1;
                                this.$set(this.formDatas1, 'timestock', arr)
                            }
                        } else {
                            this.$common.showMessageModal(this, res, 1);
                        }
        ...
        async created () {
            ...
            this.szscenicname = lab;
            const option1 = await this.$common.getProductInfoByScenicId(this, val, 1);
